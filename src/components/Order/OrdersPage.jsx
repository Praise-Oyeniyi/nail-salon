    ...
    <div className=''>
        ...
        <div className='md:w-5/6 w-[90%] mx-auto mt-5 md:mt-7 h-full mb-20'>
            <div className='w-full space-y-3'>
                {!order? Array(5).fill(0).map((_, index) => (
                    <OrderSkeleton key={index} />
                ))
                :
                order?.map((e, index)=>(
                    <div className='w-full md:px-5 p-1 md:py-3 py-2 border' key={index}>
                        {/* <h4 className='font-bold md:text-xl text-base'>Order Item</h4> */}
                        <h5 className="state font-medium w-fit text-[#ff00ff] md:tracking-wider px-1 text-xs md:text-sm bg-[#fff1f5]">{e?.status}</h5>
                                ...
                                <div className='flex w-full justify-between'>
                                    <div className="left flex gap-x-2">
                                        ...
                                        <div className='text-sm'>
                                            <h6 className='item text-[#cccccc] text-xs md:sm truncate w-fit md:w-2/6'>{e?.description}</h6>
                                            ...
                                            <div className="colors gap-x-1 text-xs flex font-medium">
                                                ...
                                                <button className='border border-[#cccccc] px-2 max-w-fit'>{e?.color}</button>
                                    ...
                                    <div className="right self-end flex items-center justify-center gap-x-1 md:gap-x-3 text-xs md:text-sm">
                                        <button className='amount border border-gray-700 w-16 md:w-20 rounded-md text-center font-medium'>{e?.quantity} piece</button>
                                        <p className='cost border border-gray-600 w-12 md:w-16 rounded-md text-center font-medium'>${e?.final_price}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
