...
const CartPage = () => {
    ...
    const validateCoupon = async () => {
        ...
        try {
            ...
            const result = await sendApi({ code: couponCode }, 'https://wittynailtip.com/backend/valid-coupon.php');
    ...
    <div className='font-jost relative'>
        ...
        <div className='md:w-5/6 w-[90%] mx-auto mt-5 md:mt-7 h-full mb-20'>
            <div className='w-full'>
                {cart?.map((e, index)=>(
                    <div key={index}>
                        <CartItem price={e.price?.unit_amount} size={e.size}
                        cartId={e.cart_id} id={e.product_id} 
                        info={e.description} count={e.quantity} 
                        name={e.name} color={e.color} image={e.images?.[0] || ''} />
                    </div>
                ))}
            </div>
        ...
        <div className="cart-footer flex items-center w-full fixed 
        bottom-0 h-16 bg-[#fff1f5] shadow-[0_-4px_7px_-1px_rgba(0,0,0,0.1)]">
            {cart?.length > 0?
                <div className='md:w-5/6 w-[95%] mx-auto flex items-center justify-between z-10 py-2'>
                    <div className={`${cart?.length >0 && '!flex' } hidden md:text-lg 
